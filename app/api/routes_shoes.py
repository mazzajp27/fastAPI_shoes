...
from app.security.security import get_current_vendedor, get_current_admin, get_current_cliente
...
def create_shoe(shoe: ShoesCreate, db: Session = Depends(get_db), current_vendedor: Usuarios = Depends(get_current_vendedor)):
...
def read_my_shoes(db: Session = Depends(get_db), current_vendedor: Usuarios = Depends(get_current_vendedor)):
...
def update_shoe(id_shoe: int, shoe: ShoesUpdate, db: Session = Depends(get_db), current_vendedor: Usuarios = Depends(get_current_vendedor)):
...
def delete_shoe(id_shoe: int, db: Session = Depends(get_db), current_vendedor: Usuarios = Depends(get_current_vendedor)):
    db_shoe = db.query(Shoes).filter(Shoes.id_shoe == id_shoe, Shoes.vendedor_id == current_vendedor.id).first()
    if db_shoe is None:
...
def relacoes_tenis(id_shoe: int, db: Session = Depends(get_db), current_admin: Usuarios = Depends(get_current_admin)):
    ...
    return {
        ...
        "clientes": [db.query(Usuarios).filter(Usuarios.id == c.cliente_id).first().nome for c in clientes]
    }